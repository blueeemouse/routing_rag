...
##############################################################
# # 为什么啊？在导入所有的naive_rag类之前先增加这一段，就能避免配置出问题了？
# # 不过这也是进一步坐实了确实是api调用的时候，url出错了，用成了默认的openai的url
# # 是不是因为这里最先进行配置，所以后面的模型，包括llm，也都沿用这个了？
...
if __name__ == "__main__":
    ...
    hotpot_jsonl_path = r"D:\Develop\all_RAG\routing_rag\HotpotQA\hotpot_1000_samples.jsonl"
    ...
    if True:
        ...
        # 4. 尝试保存索引
        STORAGE_DIR = f"./naive_rag_index_storage_{num_samples_to_use}_samples" # 为不同采样数创建不同目录
        # save_index_to_disk(naive_rag_instance.index, storage_dir=STORAGE_DIR)

        # 在 load_index_from_storage 之前打印 Settings
        from llama_index.core import Settings
        print("加载前的 embed_model:", Settings.embed_model)
