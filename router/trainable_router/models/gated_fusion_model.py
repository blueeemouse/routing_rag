"""
GatedFusionRouterModel实现

基于门控机制融合语义特征+手工特征的路由模型

核心思想（P1方案）：
- 问题长度等统计特征是关键特征，但简单的concat无法充分利用
- 门控机制让模型自适应决定关注语义还是统计特征
...
class GatedFusionBlock(nn.Module):
    ...
    将语义特征和统计特征通过门控机制融合：
    1. 将统计特征投影到与语义特征相同的维度
    2. 计算门控权重：基于语义特征和统计特征的拼接
    3. 融合：fused = semantic * gate + stat_emb * (1 - gate)
    """
    
    def __init__(self, hidden_size: int, stat_dim: int, dropout: float = 0.1):
        """
        初始化门控融合模块
        ...
        # 统计特征投影层：将统计特征投影到hidden_size维度
        self.stat_proj = nn.Sequential(
            nn.Linear(stat_dim, hidden_size),
            nn.LayerNorm(hidden_size),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        
        # 门控网络：基于语义+统计特征计算门控权重
        # 输入：语义特征 + 统计特征（原始维度）
        self.gate = nn.Sequential(
            nn.Linear(hidden_size + stat_dim, hidden_size),
            nn.LayerNorm(hidden_size),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, semantic_emb: torch.Tensor, stat_features: torch.Tensor) -> torch.Tensor:
        """
        前向传播
        
        Args:
            semantic_emb: 语义embedding, shape: (batch_size, hidden_size)
            stat_features: 统计特征, shape: (batch_size, stat_dim)
            
        Returns:
            融合后的特征, shape: (batch_size, hidden_size)
        """
        # 1. 投影统计特征
        stat_emb = self.stat_proj(stat_features)  # (B, hidden_size)
        
        # 2. 计算门控权重
        combined = torch.cat([semantic_emb, stat_features], dim=-1)  # (B, hidden_size + stat_dim)
        gate = self.gate(combined)  # (B, hidden_size)
        
        # 3. 门控融合
        fused = semantic_emb * gate + stat_emb * (1 - gate)
        
        return fused


class GatedFusionRouterModel(BaseRouterModel):
    """
    门控融合路由模型
    
    流程：
    1. query → encoder → semantic_vec (768-d for BGE, 384-d for MiniLM)
    2. query → feature_extractor → handcrafted_vec (63-d)
    3. GatedFusion(semantic_vec, handcrafted_vec) → fused_vec
    ...
    def __init__(self, config: ModelConfig, **kwargs):
        ...
        # 门控融合模块
        self.gated_fusion = GatedFusionBlock(
            hidden_size=self.semantic_dim,
            stat_dim=self.handcrafted_dim,
            dropout=0.1
        )
        
        # 分类器
        self.classifier = nn.Linear(self.semantic_dim, self.num_strategies)
        
        # 初始化权重
        self._init_weights()
        
        print(f"GatedFusionRouterModel 初始化完成:")
    ...
    def _init_backbone(self, backbone_name: str, **kwargs):
        ...
        try:
            ...
            print(f'✓ GatedFusionRouterModel使用transformers加载: {backbone_name}')
    ...
    def forward(
        ...
        # 3. 门控融合
        fused_features = self.gated_fusion(semantic_features, handcrafted_features)  # (B, hidden_size)
        
        # 4. 可选：残差连接
        if self.use_residual:
            fused_features = fused_features + semantic_features
        
        # 5. 分类
        logits = self.classifier(fused_features)  # (B, num_strategies)
    ...
    def get_strategy_embeddings(self) -> Optional[torch.Tensor]:
        """
        获取策略embedding（GatedFusionRouterModel不使用策略embedding）
    ...
    def compute_similarity(self, query_emb: torch.Tensor, strategy_embs: torch.Tensor) -> torch.Tensor:
        """
        计算相似度（GatedFusionRouterModel不使用相似度计算）
        
        Raises:
            NotImplementedError
        """
        raise NotImplementedError(
            "GatedFusionRouterModel does not use similarity-based routing. "
...
# 注册到工厂
TrainableRouterFactory.register_model('gated_fusion')(GatedFusionRouterModel)
