...
class InternalRepresentationTrainer(BaseTrainer):
    """
    内部表征路由器训练器
    ...
    def __init__(
        ...
        Args:
            model: InternalRepresentationRouterModel 模型
            config: 配置
            output_dir: 输出目录
            logger: 日志记录器
        """
        ...
        print(f"InternalRepresentationTrainer 初始化完成")
    ...
    def compute_loss(self, batch) -> torch.Tensor:
        ...
        # 前向传播
        logits = self.model.forward(representation)  # (B, num_strategies)
    ...
    def evaluate(self, dataloader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Evaluating", ascii=True):
                ...
                # 前向传播
                logits = self.model.forward(representation)
...
# 注册到工厂
TrainableRouterFactory.register_trainer('internal_representation')(InternalRepresentationTrainer)
TrainableRouterFactory.register_trainer('internal_rep')(InternalRepresentationTrainer)
